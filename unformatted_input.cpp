...
int main()
{
	...
	cin.get(c);					// [basic_iostream&, ZNAK, -, TAK, -]			zapisuje pobrany znak pod c
	...
	//cin.get(str_buff);		// [basic_istream&, BUFOR, '\n', TAK, (X)()]	pobiera co najwyzej n znakow
	//cin.get(str_buff, delim);	// [basic_istream&, BUFOR, ?, TAK, (X)()]		pobiera co najwyzej n znakow
	...
	cin.unget();				// [basic_iostream&, ZNAK, -, -]			odklada do strumienia ostatnio pobrany znak

	cin.putback(c);				// [basic_istream, -, -, -]					odklada znak do strumienia
	...
	cin.read(buff, n);			// [basic_istream&, LANC, -, NIE, -]		pobiera dokladnie n znakow
	cin.readsome(buff, n);		// [streamsize, LANC, -, -]					pobiera co najwyzej n znakow dostepnych natychmiast
	cin.gcount();				// [stream_size, -, -, -]					zwraca liczbe odczytanych znakow przy ostatniej operacji nieformatowanego odczytu
